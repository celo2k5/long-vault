...
export async function createApp(options={}){
 ...
 const applySettings=settings=>{env.SOLANA_RPC_URL=settings.rpcUrl;env.JUPITER_API_KEY=settings.jupiterKey;env.LIVE_TRADING_ENABLED=settings.liveTrading?'true':'false';env.PERPS_TEST_MODE=settings.testMode?'true':'false';env.PERPS_TEST_FUNDING='SOL';};
 ...
 const connectionSettings=()=>({rpcConfigured:!!env.SOLANA_RPC_URL,jupiterKeyConfigured:!!env.JUPITER_API_KEY,liveTrading:env.LIVE_TRADING_ENABLED==='true',testMode:env.PERPS_TEST_MODE==='true',testFunding:'SOL',storageError:settingsStorageError});
 ...
 const server=createServer(async(req,res)=>{
  ...
  try{
   ...
   if(url.pathname==='/api/settings'){
    ...
    const next={rpcUrl:input.rpcUrl?.trim()||env.SOLANA_RPC_URL||'',jupiterKey:input.clearJupiterKey?'':input.jupiterKey?.trim()||env.JUPITER_API_KEY||'',liveTrading:input.liveTrading,testMode:input.testMode??(env.PERPS_TEST_MODE==='true'),testFunding:'SOL'};

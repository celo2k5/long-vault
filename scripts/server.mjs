...
export async function createApp(options={}){
 ...
 const applySettings=settings=>{env.SOLANA_RPC_URL=settings.rpcUrl;env.JUPITER_API_KEY=settings.jupiterKey;env.LIVE_TRADING_ENABLED=settings.liveTrading?'true':'false';env.PERPS_TEST_MODE=settings.testMode?'true':'false';env.PERPS_TEST_FUNDING=settings.testFunding==='SOL'?'SOL':'USDC';};
 ...
 const connectionSettings=()=>({rpcConfigured:!!env.SOLANA_RPC_URL,jupiterKeyConfigured:!!env.JUPITER_API_KEY,liveTrading:env.LIVE_TRADING_ENABLED==='true',testMode:env.PERPS_TEST_MODE==='true',testFunding:env.PERPS_TEST_FUNDING==='SOL'?'SOL':'USDC',storageError:settingsStorageError});
 ...
 const server=createServer(async(req,res)=>{
  ...
  try{
   ...
   if(url.pathname==='/api/settings'){
    ...
    const next={rpcUrl:input.rpcUrl?.trim()||env.SOLANA_RPC_URL||'',jupiterKey:input.clearJupiterKey?'':input.jupiterKey?.trim()||env.JUPITER_API_KEY||'',liveTrading:input.liveTrading,testMode:input.testMode??(env.PERPS_TEST_MODE==='true'),testFunding:input.testFunding||(env.PERPS_TEST_FUNDING==='SOL'?'SOL':'USDC')};
    ...
    if(setupBusy||((next.testFunding!==(env.PERPS_TEST_FUNDING==='SOL'?'SOL':'USDC')||next.testMode!==(env.PERPS_TEST_MODE==='true')||next.rpcUrl!==(env.SOLANA_RPC_URL||'')||next.jupiterKey!==(env.JUPITER_API_KEY||''))&&(live.journal.active()||live.hasUnsettledCycle())))return send(409,{error:'Finish the current cycle before replacing connections. You can still disable live trading.'});

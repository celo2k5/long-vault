...
import {PERPS,TOKEN,ATA,USDC,SOL,associated,discriminator} from './perps-policy.mjs';
...
export function closeReceipt(t,owner,request,mint=USDC){
 if(!t?.meta||t.meta.err)return 0n;
 const keys=transactionKeys(t),source=associated(request,mint).toBase58(),destination=associated(owner,mint).toBase58();
 let paid=0n;
 for(const group of t.meta.innerInstructions||[])for(const ix of group.instructions){
  ...
  if(d.length===10&&d[0]===12&&a[0]===source&&a[1]===mint.toBase58()&&a[2]===destination&&a[3]===request&&d[9]===(mint.equals(USDC)?6:mint.equals(SOL)?9:8))paid+=d.readBigUInt64LE(1);
 }
 if(!paid)return 0n;
 check(keys.includes(PERPS.toBase58()),'Close receipt is missing the perpetuals program.');
 const delta=tokenDelta(t,destination,mint.toBase58(),owner);return delta>0n?(paid<delta?paid:delta):0n;
}
...
export function swapApi(env,path,body){
 const operation=swapQueue.then(async()=>{
  for(let attempt=0;attempt<3;attempt++){
   ...
   check(r.ok,'Jupiter buyback quote unavailable. The funds remain in the source token.');const text=await r.text();check(text.length<1000000,'Buyback response exceeds size limit.');return JSON.parse(text);
  }
 });
 swapQueue=operation.catch(()=>{});return operation;
}
export function inspectSwap(ix,owner,mint,destination,amount,quote,slippage,inputMint=USDC){
 ...
 const source=associated(owner,inputMint).toBase58(),j=JUP.toBase58();
 check(k[0]===TOKEN.toBase58(),'Unexpected input token program.');
 if(shared)check(k[2]===owner&&k[3]===source&&k[6]===destination&&k[7]===inputMint.toBase58()&&k[8]===mint&&k[9]===j,'Buyback destination or authority mismatch.');
...
export async function prepareBuyback(connection,env,c,amount,api=(p,b)=>swapApi(env,p,b),inputMint=USDC){
 check(amount>0n&&amount<=BigInt(Number.MAX_SAFE_INTEGER),'Invalid realized-profit amount.');
 const owner=c.vault,mint=new PublicKey(c.tokenMint);check(!mint.equals(inputMint),'The output token cannot equal the funding token.');
 ...
 const destination=tokenAta(owner,mint,mintInfo.owner),source=associated(owner,inputMint);
 ...
 check(validAccount(input,inputMint,TOKEN)&&input.data.readBigUInt64LE(64)>=amount,'Insufficient verified settlement-token profit balance.');
 ...
 const quote=await api('quote?'+new URLSearchParams({inputMint:inputMint.toBase58(),outputMint:mint.toBase58(),amount:String(amount),slippageBps:String(c.slippageBps),swapMode:'ExactIn',instructionVersion:'V1',restrictIntermediateTokens:'true'}));
 check(quote.inputMint===inputMint.toBase58()&&quote.outputMint===c.tokenMint&&quote.inAmount===String(amount)&&quote.swapMode==='ExactIn'&&quote.slippageBps===c.slippageBps&&!Number(quote.platformFee?.feeBps),'Jupiter quote does not match the authorized buyback.');
 ...
 inspectSwap(swap,owner,c.tokenMint,destination.toBase58(),amount,quote,c.slippageBps,inputMint);
 ...
 check(!simulation.value.err&&simulation.value.accounts?.every(Boolean),'Buyback simulation failed. Funds remain in the source token.');
 ...
 return {tx,lastHeight:latest.lastValidBlockHeight,expected:{kind:'buyback',owner,sourceMint:inputMint.toBase58(),mint:c.tokenMint,source:source.toBase58(),destination:destination.toBase58(),amount:String(amount),minimum:String(minimum),requests:[]}};
}

// Unwrap only this wallet's canonical WSOL account; never transfer to a third party.
export async function prepareUnwrap(connection,owner){
 const source=associated(owner,SOL),wallet=new PublicKey(owner),info=await connection.getAccountInfo(source);
 check(info?.owner.equals(TOKEN)&&info.data.length===165&&new PublicKey(info.data.subarray(0,32)).equals(SOL)&&new PublicKey(info.data.subarray(32,64)).equals(wallet)&&info.data.readUInt32LE(72)===0&&info.data.readUInt32LE(129)===0,'Invalid wrapped SOL authority.');
 const amount=info.data.readBigUInt64LE(64);check(amount>0n,'No wrapped SOL to unwrap.');
 const latest=await connection.getLatestBlockhash('confirmed');
 const tx=new VersionedTransaction(new TransactionMessage({payerKey:wallet,recentBlockhash:latest.blockhash,instructions:[new TransactionInstruction({programId:TOKEN,keys:[{pubkey:source,isSigner:false,isWritable:true},{pubkey:wallet,isSigner:false,isWritable:true},{pubkey:wallet,isSigner:true,isWritable:false}],data:Buffer.from([9])})]}).compileToV0Message());
 const fee=await connection.getFeeForMessage(tx.message);check(fee.value!==null&&fee.value<=10000,'Unwrap fee exceeds limit.');
 const before=await connection.getBalance(wallet),sim=await connection.simulateTransaction(tx,{sigVerify:false,accounts:{encoding:'base64',addresses:[owner,source.toBase58()]}});
 check(!sim.value.err&&sim.value.accounts?.[0]?.lamports>=before+info.lamports-fee.value&&!sim.value.accounts?.[1],'SOL unwrap simulation failed.');
 return {tx,lastHeight:latest.lastValidBlockHeight,expected:{kind:'unwrap',owner,source:source.toBase58(),amount:String(amount),requests:[]}};
}
export function verifyUnwrapReceipt(t,e){
 check(t?.meta&&!t.meta.err,'Unwrap receipt unavailable.');const keys=transactionKeys(t),src=keys.indexOf(e.source),dst=keys.indexOf(e.owner);
 check(src>=0&&dst>=0&&t.meta.postBalances[src]===0&&BigInt(t.meta.preBalances[src])>=BigInt(e.amount)&&t.meta.postBalances[dst]-t.meta.preBalances[dst]+t.meta.fee===t.meta.preBalances[src],'Unwrap receipt does not return the SOL to its owner.');
}

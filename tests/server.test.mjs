...
test('Admin connection settings are encrypted, redacted, applied immediately and restored on restart',async()=>{
 ...
 try{
  ...
  const response=await send({rpcUrl:rpc,jupiterKey:apiKey,liveTrading:true,testMode:true,testFunding:'SOL'});assert.equal(response.status,200);const result=await response.json();assert.equal(result.liveTrading,true);assert.equal(result.testMode,true);assert.equal(result.testFunding,'SOL');
  const testWallet=Keypair.generate();const setup=await request('/api/setup',{method:'POST',headers,body:JSON.stringify({tokenMint:'',privateKey:JSON.stringify([...testWallet.secretKey]),cycleSeconds:120})});assert.equal(setup.status,200);
  const view=await(await request('/api/vault',{headers})).json();assert.equal(view.testMode,true);assert.equal(view.config.tokenMint,'');assert.equal(result.rpcConfigured,true);assert.equal(result.jupiterKeyConfigured,true);assert.equal(JSON.stringify(result).includes('private-'),false);
  ...
  const restored=await(await request('/api/settings',{headers:{Cookie:relogin.headers.get('set-cookie').split(';')[0]}})).json();assert.equal(restored.rpcConfigured,true);assert.equal(restored.jupiterKeyConfigured,false);assert.equal(restored.liveTrading,false);assert.equal(restored.testMode,true);assert.equal(restored.testFunding,'SOL');

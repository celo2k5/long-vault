...
test('expired signatures remain unresolved and block duplicate spending',async()=>{
 ...
 try{await service.execute({kind:'resume'});await service.execute({kind:'open',market:'SOL'},'expired-order');assert.equal(service.journal.get('expired-order').status,'unknown');assert.equal(service.journal.paused(),true);await assert.rejects(service.execute({kind:'resume'}),/Cannot resume while a transaction is unresolved.*Signature not found after expiry/);assert.equal(service.journal.paused(),true);assert.equal(service.journal.list().length,1);}finally{service.close();db.close();}
});

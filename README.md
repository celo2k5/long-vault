...
In Admin → Connections, enable **Position test mode** and allow live trading, then save. In Setup, save a developer wallet with the token CA blank. Tests use SOL funding. Fund approximately $10 worth per position plus the 0.04 SOL reserve; the amount is calculated from the current SOL quote. Review leverage and TP/SL in Strategy, then click **Enable position tests** and manually open BTC, ETH or SOL.

These are real mainnet positions, not simulated trades. Each open forces $10 worth of SOL rounded up to the nearest lamport, regardless of caller-supplied amounts; the normal notional cap and quote/liquidation checks still apply. Test mode does not claim rewards, buy back tokens, or start automatic cycles. Manual closes and on-chain TP/SL remain available. Closed test proceeds are converted to wrapped SOL and then unwrapped into the wallet after finalized receipt checks; no buyback runs in test mode. Close and reconcile test positions before switching modes or changing wallet/connection settings. After returning to normal mode, a valid CA and verified fee creator are required again.
...
3. Budget the entire cycle once. Use SOL directly as Jupiter input, preserving a 0.15 SOL cycle reserve. Jupiter performs any required collateral conversion. Existing USDC is not counted as new-cycle funding.
...
5. After each close, scan finalized payout receipts from the recorded Jupiter request escrow. Convert verified BTC, ETH or USDC payouts to wrapped SOL using an independently bounded ExactIn swap. Once finalized SOL receipts recover the whole cycle principal and a conservative SOL fee/rent reserve, swap the saved buyback percentage of the SOL surplus (default 75%) into the saved token CA. After all legs and buybacks settle, unwrap remaining SOL into the wallet. The new tokens stay in the developer wallet. This is a market buy, not a burn or a guaranteed chart price increase.
...
### Protected closes and SOL settlement (September 25, 2026)

Manual and automatic fallback closes now build `createDecreasePositionMarketRequest` locally, returning the market's native collateral (BTC, ETH or SOL) to the developer wallet. There is no collateral swap inside this request. The configured minimum closing price and full-position constraint are inspected before simulation and signing. This bypasses the unsupported instant close-and-swap route described above; that route remains blocked.

The keeper watches stored TP/SL prices for app-managed positions and queues at most one fallback close when crossed. On-chain TP/SL orders remain independent. Request submission is not a fill: the journal waits for finality, request execution and a zero position size. If the request settles with a remaining position, it pauses for reconciliation.

Jupiter's API rejects direct SOL TP/SL payouts for BTC and ETH. Their TP/SL may pay USDC; local close requests pay their collateral token. Finalized receipts are tracked by mint, converted to SOL with saved swap slippage and a 3% price-impact cap, and verified again from finalized token deltas. Only then can SOL profit fund a buyback. Separate settlement and unwrap orders retain signed bytes across restarts. Failures preserve funds, pause automation and require review/resume; ambiguous submissions cannot be replaced. Keep native SOL for network fees even while proceeds are wrapped.

New cycles account for principal, reserves and profit in lamports. Cycles already persisted without a currency marker finish using their original USDC accounting to avoid reinterpreting units. Existing orders are not rewritten or re-signed by the migration.

Validation: an unsigned native-collateral BTC close request passed mainnet simulation on September 25; an unsigned USDC-to-SOL quote passed the swap policy. Regression tests cover native close bounds/destinations, target-triggered close idempotency, multi-stage SOL profit accounting, restart behavior, receipt authority and SOL unwrapping. No funded end-to-end trade was submitted during development; keeper fills and conversions remain conditional on prices, liquidity, RPC availability and configured limits.

...
export default function VaultDashboard({view='overview'}:{view?:'overview'|'admin'}){
 ...
 {admin?<div className="admin-heading"><form method="post" action="/admin/logout"><button className="text-button">Sign out</button></form><span className="eyebrow">LONG CONTROL</span><h1>Admin</h1><p>One token. One developer wallet. One cycle.</p></div>:<div className="hero"><div className="token-emblem"><img src="/long-pfp.png" alt="$LONG profile icon" width={1254} height={1254}/></div><h1>$LONG</h1><p>BTC, ETH &amp; SOL. One wallet.<br/><span>Creator rewards. Long positions. $LONG.</span></p><div className="address"><label>DEVELOPER WALLET</label><span>{config.vault||'Not configured'}</span>{config.vault&&<button aria-label="Copy developer wallet address" onClick={()=>copy(config.vault)}><Copy size={12}/></button>}</div></div>}

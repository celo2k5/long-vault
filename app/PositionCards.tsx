...
export function PositionCards({positions,leverage,unavailable=false,prices={}}:{positions:CardPosition[];leverage:number;unavailable?:boolean;prices?:Partial<Record<Market,number>>}){
 return <div className="positions sleek-positions">{MARKETS.map(market=>{
  ...
  return <article className={'position sleek-position '+(loss?'losing':p&&p.pnlUsd>0?'winning':'neutral')} key={market} aria-label={market+' position'}>
   <div className="coin-orbit"><img src={'/coins/'+market.toLowerCase()+'.svg'} alt={names[market]+' logo'} width="42" height="42"/></div>
   <div className="position-head"><div><h3>{market}</h3><span>{names[market]}{p?.side==='short'?' · Short':''}</span></div><b className="badge">{p?Number(p.leverage.toFixed(1)):leverage}×</b></div>
   <div className={'pnl '+(loss?'negative':p?'positive':'')}>{p?(p.pnlUsd>0?'+':'')+currency(p.pnlUsd):unavailable?'—':'$0.00'}</div>
   <div className={'gain '+(loss?'negative':'positive')}>{p?(p.roe>0?'+':'')+p.roe.toFixed(1)+'% ROE':unavailable?'Data unavailable':'Awaiting cycle'}</div>
   <div className="mini-chart">{values.length>1?<svg viewBox="0 0 220 48" role="img" aria-label={market+' observed price trend'}><polyline points={points} fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinejoin="round" strokeLinecap="round"/></svg>:<span>{active?'Collecting price history':'No active price history'}</span>}</div>
   ...
   <div className="card-caption"><span>{p?currency(p.notionalUsd)+' notional':'Waiting for capital'}</span><span>{progress===null?'TP —':Math.round(progress)+'% to TP'}</span></div>
   <footer><span className="status-dot"/>{unavailable?'Connection unavailable':p?.status==='liquidated'?'Liquidated':active?'Open · '+distance.toFixed(1)+'% to liquidation':p?'Closed':'Waiting'}{p?.note&&<span title={p.note}> · {p.note}</span>}</footer>
  </article>;
 })}</div>;
}

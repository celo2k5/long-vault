...
export async function readMainnet(config:Config,env:LiveEnvironment):Promise<MainnetReport>{
 ...
 r.checks.push({name:'Automated execution',status:'blocked',detail:'Signing, broadcast reconciliation and live profit accounting are not enabled. Connection checks and previews cannot move funds.'});
